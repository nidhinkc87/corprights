  ...
  return (
    <li className="relative xl:py-8 group cursor-pointer">
      <div className="max-xl:hidden">
        <span className="flex justify-between gap-x-2 items-center px-4 py-4 xl:py-2 text-xl md:text-base font-bold text-dark group-hover:text-primary hover:bg-gray-100/10 ease-linear duration-300  peer">
          Services
          <svg
          ...
          <ScrollArea className="max-h-72 w-48 overflow-y-auto">
            {services?.map((service) => (
              <li
                ...
                className={cn(
                  "relative group/child cursor-pointer p-4 lg:hover:bg-gray-100/10  ease-linear duration-300",
                  pathname === `/services/${service?.slug}` && "bg-gray-100/10"
                )}
              >
                <Link
                  href={`/services/${service?.slug}`}
                  className={cn(
                    "text-base font-bold text-dark group-hover/child:text-primary ease-linear duration-300 py-4 w-full",
                    pathname === `/services/${service?.slug}` && "text-primary"
                  )}
                >
                  {service?.title}
                </Link>
      ...
      <Collapsible className="xl:hidden">
        <CollapsibleTrigger className="w-full flex justify-between gap-x-2 items-center px-4 py-4 xl:py-2 text-xl md:text-base font-bold text-dark group-hover:text-primary hover:bg-gray-100/10 ease-linear duration-300  peer [&[data-state=open]>svg]:rotate-180">
          Services
          <svg
        ...
        <CollapsibleContent className="max-xl:ml-4 xl:absolute xl:pt-8 bg-white xl:p-6">
          <ScrollArea className="max-h-72 w-full overflow-y-auto">
            {services?.map((service) => (
              <li
                key={`service-${service?.id}`}
                className={cn(
                  "relative group/child cursor-pointer p-4 hover:bg-gray-100/10  ease-linear duration-300 ",
                  pathname === `/services/${service?.slug}` && "bg-gray-100/10"
                )}
              >
                <Link
                  href={`/services/${service?.slug}`}
                  className={cn(
                    "text-base font-bold text-dark group-hover/child:text-primary ease-linear duration-300 py-4",
                    pathname === `/services/${service?.slug}` && "text-primary"
                  )}
                >
                  {service?.title}
