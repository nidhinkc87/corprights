...
import LicenseDropdown from "@/components/sections/layout/header/LicenseDropdown";
import Link from "next/link";
import ServicesDropdown from "@/components/sections/layout/header/ServicesDropdown";
...
const MobileNavMenu = () => {
  ...
  return (
    <>
      ...
      <nav
        className={cn(
          "flex-col max-xl:absolute inset-x-0 top-[71px] bg-white  w-full transition-all duration-300 ease-in-out max-xl:overflow-y-auto xl:hidden",
          menuOpen ? "h-[calc(100vh-71px)]" : "h-0"
        )}
        id="toggle-menu"
      >
        <ul className="max-xl:container flex-1 flex flex-col xl:items-center  pt-6 ">
          <ServicesDropdown />
          ...
          {paths?.map((path) => (
            ...
            >
              <Link
                href={path?.path ?? "#"}
                className={cn(
                  "flex justify-between gap-x-2 items-center px-4 py-4 xl:py-2 text-xl md:text-base font-bold text-dark group-hover:text-primary hover:bg-gray-100/10 ease-linear duration-300",
